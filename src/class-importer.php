...
class Importer {
	...
	public function import_item( array $data, $parent_post_id = 0, $import_ignored = false, array $arg_overrides = array() ) {
		...
		if ( ! $import_ignored && wp_list_filter( $tags, array( 'name' => 'ignore' ) ) ) {
		...
		if ( wp_list_filter( $tags, array( 'name' => 'ignore' ) ) ) {
			return false;
		}
	...
	protected function _set_namespaces( $post_id, $data ) {
		...
		foreach ( $namespaces as $namespace ) {
			$ns_term = $this->insert_term( $namespace, $this->taxonomy_namespace, array(
	...
	protected function set_packages( $post_id, $tags ) {
		...
		$packages = array(
			'main' => wp_list_filter( $tags, array( 'name' => 'package' ) ),
			'sub'  => wp_list_filter( $tags, array( 'name' => 'subpackage' ) ),
		);
